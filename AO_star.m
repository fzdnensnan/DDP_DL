function nextAction = AO_star(muGraph,sigmaGraph,traversalGraph,currentState,destinationNode,synchronous)
...
while AOtree(1).status == false && AOtree(1).f ~= inf
    ...
    [assumptMuGraph,assumptTraversalGraph] = calculateAssumptGraphFromActionStatusList(node.state.actionStatusList,muGraph,traversalGraph);
    AOtree = backpropagate(node,AOtree,assumptMuGraph);    
end
...
if ~isempty(path)
    ...
    function path = outputPath(AOtree,root,destinationNode)
        [~,treeSize] = size(root);
...
%             path = [path,AOtree(nodeIndex).state.nodeId];
    ...
    function AOtree = backpropagate(node,AOtree,assumptMuGraph)
